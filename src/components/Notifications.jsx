    ...
    <>
      ...
      {invites.length > 0 && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-40 w-full max-w-md px-4 space-y-2">
          {invites.map(inv => (
            <div key={inv.id} className="card fade-in flex items-center justify-between gap-3" style={{ background: '#FAFAF7', boxShadow: '0 8px 24px rgba(0,0,0,0.06)' }}>
      ...
      <div className="fixed bottom-6 right-6 z-40 space-y-2">
        {toasts.map(t => (
          <div key={t.id} className="card fade-in font-mono text-xs tracking-wide" style={{
            background: '#FAFAF7',
            color: t.type === 'error' ? '#B91C3C' : t.type === 'success' ? '#2F6B3F' : '#1A1A1A',
